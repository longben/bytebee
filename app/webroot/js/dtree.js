...
dTree.prototype.node = function(node, nodeId) {
	...
	if (this.config.useIcons) {
		...
		if(this.config.check==true){
			str+= '<img checked=0  src="'+this.icon.iconUnCheckAll+'" value='+node.id+' id="c'+  this.obj + nodeId + '" onclick="javascript:'+this.obj+'.cc('+nodeId+')" border="0"/>';
    ...
    if(node.nodeActionList){
        ...
        for(i=0;i<node.nodeActionList.size();i++){
            ...
            str += '<img id="i' + this.obj + nodeId + '" src="' + ((nodeActionObj.actionIcon) ? nodeActionObj.actionIcon : node.icon)
                    + '" alt="'+nodeActionObj.actionTitle+'" border="0"/>';
            str+='</a>';
        }
...
dTree.prototype.indent = function(node, nodeId) {
	...
	if (this.root.id != node.pid) {
		...
		} else str += '<img src="' + ( (this.config.useLines) ? ((node._ls) ? this.icon.joinBottom : this.icon.join ) : this.icon.empty) + '" alt="" border="0"/>';
